...
CONF_CONFIG_BUTTON = "apply_config"
CONF_REVERT_BUTTON = "revert_config"
CONF_RESTART_BUTTON = "restart_module"
CONF_RESET_BUTTON = "factory_reset"


CONFIG_SCHEMA = {
    cv.GenerateID(CONF_LD2420_ID): cv.use_id(LD2420Component),
    cv.Required(CONF_CONFIG_BUTTON): button.button_schema(
...
async def to_code(config):
    ...
    if apply_config := config.get(CONF_CONFIG_BUTTON):
    ...
    if revert_config := config.get(CONF_REVERT_BUTTON):
    ...
    if factory_reset := config.get(CONF_RESET_BUTTON):
