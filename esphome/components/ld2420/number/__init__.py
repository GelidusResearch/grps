...
CONF_MIN_DISTANCE_GATE = "min_gate_distance"
CONF_MAX_DISTANCE_GATE = "max_gate_distance"
...
CONFIG_SCHEMA = {
    ...
    cv.Inclusive(CONF_MIN_DISTANCE_GATE, TIMEOUT_GROUP): number.number_schema(
        LD2420MinDistanceNumber,
        device_class=DEVICE_CLASS_DISTANCE,
        entity_category=ENTITY_CATEGORY_CONFIG,
        icon=ICON_MOTION_SENSOR,
    ),
    cv.Inclusive(CONF_MAX_DISTANCE_GATE, TIMEOUT_GROUP): number.number_schema(
...
async def to_code(config):
    ...
    if gate_config_group := config.get(CONF_TIMEOUT):
        n = await number.new_number(
            gate_config_group, min_value=0, max_value=65535, step=15
        )
