...
void LD2420MinDistanceNumber::control(float value) {
  const uint16_t min_distance = (uint16_t) value;
  this->publish_state(value);
  this->parent_->new_config_.min_gate = min_distance;
}

void LD2420MaxDistanceNumber::control(float value) {
  const uint16_t max_distance = (uint16_t) value;
  this->publish_state(value);
  this->parent_->new_config_.max_gate = max_distance;
}
